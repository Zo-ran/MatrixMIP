...
check_input <- function(Initial_X, A, B, K, Tr, Scalar, Bscalar, Gpp, ...) {
  ...
  check_matrix <- function(X, name) {
    if (!is.matrix(X)) {
      ...
      stop(paste(name, "must be square"))
    }
  }
  check_matrix(A, "'A'")
  check_matrix(K, "'K'")
  ...
  if (dim(A)[2] != length(Initial_X) || dim(A)[2] != length(B))
    stop("the length of 'Initial_X', 'B' must be the same as the number of columns in 'A'")
...
check_order <- function(order, name, pool_n) {
  if (!is.null(order)) {
    if (!is.vector(order))
      stop(paste(name, "must be a vector"))
    for (i in order) {
      if (i > pool_n)
        stop(paste("the elements in", name, "cannot be greater than the number of C pool"))
    }
  }
}
